...
class CustomDataset(COCODetection):
    ...
    def __init__(self, basedir, cfg, split):
        self.cfg = cfg
        basedir = os.path.expanduser(basedir)
        
        if split in cfg.DATA.TRAIN.to_dict().keys():
            dir_dict = cfg.DATA.TRAIN.to_dict()
            
        else:
            dir_dict = cfg.DATA.VAL.to_dict()
            
        self._imgdir = os.path.realpath(
            os.path.join(basedir, dir_dict[split]['image_dir'])
        )
        
        
        assert os.path.isdir(self._imgdir), "{} is not a directory!".format(self._imgdir)
        annotation_file = os.path.join(basedir, dir_dict[split]['annot_path'])
        assert os.path.isfile(annotation_file), annotation_file
...
def register_custom_data():
    dataset_names = list(cfg.DATA.TRAIN.to_dict().keys()) + list(cfg.DATA.VAL.to_dict().keys())
    
    for split in dataset_names:
        DatasetRegistry.register(split,lambda x=split: CustomDataset(cfg.DATA.BASEDIR, cfg, x))
        DatasetRegistry.register_metadata(split,'class_names',cfg.DATA.CLASS_NAMES)
