...
class AttrDict():
    ...
    def update_from_json(self, json_path):
        """Update from json format configs. """
        with open(json_path, 'rb') as f:
            j_file = f.read()
            json_data = json.loads(j_file)

        dic = self
        self._update_dict(dic, json_data)
        dic.FPN.ANCHOR_STRIDES = tuple(dic.FPN.ANCHOR_STRIDES)
        dic.RPN.ANCHOR_RATIOS = tuple(dic.RPN.ANCHOR_RATIOS)
        dic.RPN.ANCHOR_SIZES = tuple(dic.RPN.ANCHOR_SIZES)

    def _update_dict(self, old_dict, user_dict):
        for k, v in user_dict.items():
            if isinstance(v, dict):
                sub_old_dict = getattr(old_dict, k)
                self._update_dict(sub_old_dict, v)
            else:
                oldv = getattr(old_dict, k)
                if not isinstance(oldv, str) and isinstance(v, str):
                    v = eval(v)
                setattr(old_dict, k, v)
...
# dataset -----------------------
_C.DATA.BASEDIR = './custom_data/'
# All available dataset names are defined in `dataset/coco.py:register_coco`.
# All TRAIN dataset will be concatenated for training.

_C.DATA.TRAIN.train.image_dir = "train"  # image directory for training
_C.DATA.TRAIN.train.annot_path = "annotations/train.json"  # coco-style json file path for training
# Each VAL dataset will be evaluated separately (instead of concatenated)
_C.DATA.VAL.eval.image_dir = "valid"  # image directory for evaluation
_C.DATA.VAL.eval.annot_path = "annotations/eval.json"  # coco-style json file path for evaluation
...
def finalize_configs(is_training):
    ...
    datasets = list(_C.DATA.TRAIN.to_dict().keys()) + list(_C.DATA.VAL.to_dict().keys())
