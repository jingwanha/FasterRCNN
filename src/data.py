...
def print_class_histogram(roidbs):
    ...
    """
    class_names = DatasetRegistry.get_metadata(list(cfg.DATA.TRAIN.to_dict().keys())[0], 'class_names')
...
def get_train_dataflow():
    ...
    """
    roidbs = list(itertools.chain.from_iterable(DatasetRegistry.get(x).training_roidbs() for x in cfg.DATA.TRAIN.to_dict().keys()))
