...
from dataset import register_coco, register_balloon, register_custom_data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', help='Log directory. Will remove the old one if already exists.',default='train_log/faster_rcnn')
    parser.add_argument('--config', help="config json file path")
    parser.add_argument('--export_model', help='path to save model weight',default='export')
    parser.add_argument('--proc_name', help='display process name in nvidia-smi')


    args = parser.parse_args()
    setproctitle('{}'.format(args.proc_name))
    ...
    callbacks = [
        PeriodicCallback(
            ModelSaver(max_to_keep=50, keep_checkpoint_every_n_hours=1,checkpoint_dir=args.export_model),
    ...
    # Append evaluation callback
    if cfg.TRAIN.EVAL_PERIOD > 0:
        callbacks.extend([
            EvalCallback(dataset,*MODEL.get_inference_tensor_names(),args.logdir) for dataset in cfg.DATA.VAL.to_dict().keys()
        ])
        # to keep best model checkpoint
        callbacks.append(
            PeriodicCallback(
                MaxSaver('eval-mAP(bbox)/IoU=0.5', filename='max-eval-mAP',checkpoint_dir=args.export_model), 
                every_k_epochs=cfg.TRAIN.EVAL_PERIOD
            ) 
        )
